...
gulp.task('styles:build-library-base-file', function(done){
    // This task generates the core library.scss file and all of its @import statements
    var componentConfig = yaml.load('src/library/component_config.yaml'),
        libraryScss = `// DO NOT EDIT: This file is automatically generated by the project\'s build task - ${versionStamp}\n
@import 'constants';\n\n`;

    componentConfig.shared.forEach(function(sharedObject) {
        var sharedImport = Object.keys(sharedObject)[0];
        if (sharedObject[sharedImport]) {
            libraryScss += `@import 'shared/${sharedImport}';\n`;
        }
    });

    libraryScss += '\n';

    componentConfig.components.forEach(function(componentObject){
        var componentName = Object.keys(componentObject)[0];
        if (componentObject[componentName]) {
            libraryScss += `@import '${componentName}/${componentName}';\n`;
        }
    });

    // Add print styles at the end
    libraryScss += `\n@import '${theme}_library_print';\n`;
    fs.writeFileSync(`src/library/styles/${theme}_library.scss`, libraryScss);
    done();
});
...
gulp.task('styles:compile-library', function() {
    return gulp.src('src/library/styles/*.scss')
        .pipe(sass({
            includePaths: ['src/library/components', 'src/library/styles/auto_generated']
...
gulp.task('styles:compile-doc', function() {
    return gulp.src('src/doc/assets/styles/*.scss')
        .pipe(sass({
            includePaths: ['src/library/components', 'src/library/styles', 'src/library/styles/auto_generated']
        }).on('error', sass.logError))
        .pipe(concat.header(versionStampCssJs))
        .pipe(gulp.dest('dist/assets/styles/'));
});
...
gulp.task('markup:concatenate-library-macros', function(){
    return gulp.src(['src/library/components/**/*.njk', `!src/library/components/${theme}_library_macros.njk`])
        .pipe(concat(`${theme}_library_macros.njk`))
...
gulp.task('markup:concatenate-doc-library-macros', function(){
    return gulp.src(['src/doc_library/components/**/*.njk', `!src/doc_library/components/${theme}_doc_library_macros.njk`])
        .pipe(concat(`${theme}_doc_library_macros.njk`))
        .pipe(concat.header('{# DO NOT EDIT: This file is automatically generated by the project\'s build task #}\n'))
...
function interpolate_yaml_variables(variables_object) {
    const regex = /!{\*(.*?)}/g;
...
gulp.task('constants:convert-to-scss-and-json', function(done){
    ...
    fs.writeFileSync(`src/doc/data/auto-generated/${theme}_constants.json`, jsBeautify(jsonConstants));
...
gulp.task('watch:markup:library-macros', function(){
    return gulp.watch(['src/library/components/**/*.njk', `!src/library/components/${theme}_library_macros.njk`], gulp.series('markup:concatenate-library-macros', 'markup:compile-docs', 'browser-sync-reload'));
});

gulp.task('watch:markup:doc-library-macros', function(){
    return gulp.watch(['src/doc_library/components/**/*.njk', `!src/doc_library/components/${theme}_doc_library_macros.njk`], gulp.series('markup:concatenate-doc-library-macros', 'markup:compile-docs', 'browser-sync-reload'));
});
...
gulp.task('watch:constants:library', function(){
    return gulp.watch([`src/library/constants/constants.yaml`, 'src/library/component_config.yaml'], gulp.series('constants:convert-to-scss-and-json', 'styles:build-library-base-file', 'data:build:allData', 'build:styles:all', 'build:markup:all', 'browser-sync-reload'));
});
...
gulp.task('build:data:all', gulp.series(gulp.parallel('constants:convert-to-scss-and-json', 'styles:build-library-base-file', 'data:build:icons', 'data:build:project'), 'data:build:allData'));
...
gulp.task('clean:markup:concatenated-macros', function(){
    return(del([`src/library/components/${theme}_library_macros.njk`, `src/doc_library/components/${theme}_doc_library_macros.njk`]));
});
gulp.task('clean:data', function(){
    return(del('src/doc/data/auto-generated/**/*'));
});
gulp.task('clean:library-token-files', function(){
    return(del([`src/library/styles/_${theme}_constants.scss`, `src/library/data/${theme}_constants.json`]));
});
...
gulp.task('release:compiled-assets', function(){
    return(gulp.src(['dist/assets/styles/*.css', 'dist/assets/scripts/*.js', 'dist/assets/icons/*.svg', `!dist/assets/styles/${theme}_doc*`, `!dist/assets/scripts/${theme}_doc*`]))
...
gulp.task('release:constants:json', function(){
    return(gulp.src(`src/library/data/${theme}_constants.json`))
        .pipe(gulp.dest(`releases/${project.version}/src/scripts`));
});
