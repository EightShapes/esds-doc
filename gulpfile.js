    ...
    return gulp.src('src/doc/**/*.njk')
        .pipe(
            nunjucksRender({
                ...
                data: fs.existsSync(`src/library/data/${projectName}_tokens.json`) ? JSON.parse(fs.readFileSync(`src/library/data/${projectName}_tokens.json`, {encoding: 'UTF-8'})) : {},
            }).on('error', function(e){
                gutil.log(e);
                gutil.beep();
                this.emit('end');
...
gulp.task('tokens:convert-to-scss-and-json', function(done){
    ...
    var tokens = yaml.load(`src/library/tokens/${projectName}_tokens.yaml`),
        jsonTokens = JSON.stringify({token: tokens}),
        flattenedTokens = flatten(tokens, {delimiter: '-'}),
        scss = '',
        prevVarNameParent = false;

    // Write out JSON version of tokens
    fs.writeFileSync(`src/library/data/${projectName}_tokens.json`, jsonTokens);
...
gulp.task('tokens:beautify-json', function(){
    return gulp.src(`src/library/data/${projectName}_tokens.json`)
        .pipe(prettify())
        .pipe(gulp.dest('src/library/data'));
});
...
gulp.task('watch:svgs', function(){
    return gulp.watch('src/library/icons/**/*.svg', gulp.series('build:svgs:all'));
});
...
gulp.task('watch:tokens:library', function(){
    return gulp.watch(`src/library/tokens/${projectName}_tokens.yaml`, gulp.series('tokens:convert-to-scss-and-json', 'tokens:beautify-json', 'build:styles:all', 'build:markup:all'));
});
...
gulp.task('build:project', gulp.parallel('build:styles:all', 'build:scripts:all', 'build:markup:all', 'build:svgs:all', 'images:copy-doc'));
...
// Build dist
gulp.task('build:dist', gulp.series('clean:dist', 'clean:markup:concatenated-macros', 'tokens:convert-to-scss-and-json', 'tokens:beautify-json', 'build:project'));
