...
var markerRegex$1 = new RegExp("".concat(marker$1, "|").concat(nodeMarker$1));
/**
 * Suffix appended to all bound attribute names.
 */
...
var Template$1 = function Template(result, element) {
  _classCallCheck(this, Template);
  ...
  while (partIndex < length) {
    ...
    /* Node.ELEMENT_NODE */
        ...
        if (node.hasAttributes()) {
          ...
          while (count-- > 0) {
            ...
            var name = lastAttributeNameRegex$1.exec(stringForPart)[2]; // Find the corresponding attribute
            // All bound attributes have had a suffix added in
            // TemplateResult#getHTML to opt out of special attribute
            // handling. To look up the attribute value we also need to add
            // the suffix.

            var attributeLookupName = name.toLowerCase() + boundAttributeSuffix$1;
            var attributeValue = node.getAttribute(attributeLookupName);
            node.removeAttribute(attributeLookupName);
            var statics = attributeValue.split(markerRegex$1);
            this.parts.push({
              type: 'attribute',
              index: index,
              name: name,
    ...
    /* Node.TEXT_NODE */
        ...
        if (data.indexOf(marker$1) >= 0) {
          ...
          var _strings = data.split(markerRegex$1);
          ...
          for (var _i = 0; _i < lastIndex; _i++) {
            ...
            if (s === '') {
              ...
              var match = lastAttributeNameRegex$1.exec(s);
    ...
    /* Node.COMMENT_NODE */
        ...
        if (node.data === marker$1) {
          ...
          if (node.previousSibling === null || index === lastPartIndex) {
            ...
            _parent.insertBefore(createMarker$1(), node);
          }
          ...
          while ((_i2 = node.data.indexOf(marker$1, _i2 + 1)) !== -1) {
            // Comment node has a binding marker inside, make an inactive part
            // The binding won't work, but subsequent bindings will
            // TODO (justinfagnani): consider whether it's even worth it to
            // make bindings in comments work
...
var endsWith$1 = function endsWith(str, suffix) {
  var index = str.length - suffix.length;
  return index >= 0 && str.slice(index) === suffix;
};
...
var commentMarker$1 = " ".concat(marker$1, " ");
/**
 * The return type of `html`, which holds a Template and the values from
 * interpolated expressions.
 */

var TemplateResult$1 =
/*#__PURE__*/
function () {
  function TemplateResult(strings, values, type, processor) {
    _classCallCheck(this, TemplateResult);
...
try {
  ...
  window.addEventListener('test', options$1, options$1); // tslint:disable-next-line:no-any
...
function templateFactory$1(result) {
  ...
  var key = result.strings.join(marker$1); // Check if we already have a Template for this key
...
var render$2 = function render(result, container, options) {
...
var html$1 = function html(strings) {
  ...
  return new TemplateResult$1(strings, values, 'html', defaultTemplateProcessor$1);
};
...
function removeNodesFromTemplate$1(template, nodesToRemove) {
  var content = template.element.content,
      parts = template.parts;
  var walker = document.createTreeWalker(content, walkerNodeFilter$1, null, false);
  var partIndex = nextActiveIndexInTemplateParts$1(parts);
  var part = parts[partIndex];
  var nodeIndex = -1;
  var removeCount = 0;
  var nodesToRemoveInTemplate = [];
...
var countNodes$1 = function countNodes(node) {
  var count = node.nodeType === 11
  /* Node.DOCUMENT_FRAGMENT_NODE */
  ? 0 : 1;
  var walker = document.createTreeWalker(node, walkerNodeFilter$1, null, false);
...
var nextActiveIndexInTemplateParts$1 = function nextActiveIndexInTemplateParts(parts) {
  var startIndex = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : -1;
...
function insertNodeIntoTemplate$1(template, node) {
  var refNode = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : null;
  var content = template.element.content,
      parts = template.parts; // If there's no refNode, then put node at end of template.
  // No part indices need to be shifted in this case.
...
var getTemplateCacheKey$1 = function getTemplateCacheKey(type, scopeName) {
  return "".concat(type, "--").concat(scopeName);
};

var compatibleShadyCSSVersion$1 = true;
...
var shadyTemplateFactory$1 = function shadyTemplateFactory(scopeName) {
  return function (result) {
    var cacheKey = getTemplateCacheKey$1(result.type, scopeName);
    ...
    var key = result.strings.join(marker$1);
    template = templateCache.keyString.get(key);
...
var TEMPLATE_TYPES$1 = ['html', 'svg'];
/**
 * Removes all style elements from Templates for the given scopeName.
 */

var removeStylesFromLitTemplates$1 = function removeStylesFromLitTemplates(scopeName) {
  TEMPLATE_TYPES$1.forEach(function (type) {
    var templates = templateCaches$1.get(getTemplateCacheKey$1(type, scopeName));
...
var prepareTemplateStyles$1 = function prepareTemplateStyles(scopeName, renderedDOM, template) {
  shadyRenderSet$1.add(scopeName); // If `renderedDOM` is stamped from a Template, then we need to edit that
  // Template's underlying template element. Otherwise, we create one here
  // to give to ShadyCSS, which still requires one while scoping.
...
var render$3 = function render(result, container, options) {
  if (!options || _typeof(options) !== 'object' || !options.scopeName) {
    throw new Error('The `scopeName` option is required.');
  }
  ...
  var needsScoping = compatibleShadyCSSVersion$1 && container.nodeType === 11
  /* Node.DOCUMENT_FRAGMENT_NODE */
  && !!container.host; // Handle first render to a scope specially...

  var firstScopeRender = needsScoping && !shadyRenderSet$1.has(scopeName); // On first scope render, render into a fragment; this cannot be a single
  // fragment that is reused since nested renders can occur synchronously.

  var renderContainer = firstScopeRender ? document.createDocumentFragment() : container;
  render$2(result, renderContainer, Object.assign({
    templateFactory: shadyTemplateFactory$1(scopeName)
  }, options)); // When performing first scope render,
  // (1) We've rendered into a fragment so that there's a chance to
  // `prepareTemplateStyles` before sub-elements hit the DOM
  // (which might cause them to render based on a common pattern of
  ...
  if (firstScopeRender) {
    var part = parts$1.get(renderContainer);
    ...
    var template = part.value instanceof TemplateInstance$1 ? part.value.template : undefined;
...
var notEqual$1 = function notEqual(value, old) {
  // This ensures (old==NaN, value==NaN) always returns false
  return old !== value && (old === old || value === value);
};
...
var microtaskPromise$1 = Promise.resolve(true);
var STATE_HAS_UPDATED$1 = 1;
var STATE_UPDATE_REQUESTED$1 = 1 << 2;
var STATE_IS_REFLECTING_TO_ATTRIBUTE$1 = 1 << 3;
var STATE_IS_REFLECTING_TO_PROPERTY$1 = 1 << 4;
var STATE_HAS_CONNECTED$1 = 1 << 5;
/**
 * The Closure JS Compiler doesn't currently have good support for static
 * property semantics where "this" is dynamic (e.g.
 * https://github.com/google/closure-compiler/issues/3177 and others) so we use
...
function (_HTMLElement) {
  ...
  function UpdatingElement() {
    ...
    _this._updateState = 0;
    _this._instanceProperties = undefined;
    _this._updatePromise = microtaskPromise$1;
    _this._hasConnectedResolver = undefined;
    /**
     * Map with keys for any properties that have changed since the last
     * update cycle with previous values.
  ...
  _createClass(UpdatingElement, [{
    ...
    key: "connectedCallback",
    value: function connectedCallback() {
      this._updateState = this._updateState | STATE_HAS_CONNECTED$1; // Ensure first connection completes an update. Updates cannot complete
      // before connection and if one is pending connection the
      // `_hasConnectionResolver` will exist. If so, resolve it to complete the
      // update, otherwise requestUpdate.
    ...
    value: function _attributeToProperty(name, value) {
      ...
      if (propName !== undefined) {
        var options = ctor._classProperties.get(propName) || defaultPropertyDeclaration$1; // mark state reflecting
    ...
    value: function _requestUpdate(name, oldValue) {
      ...
      if (name !== undefined) {
        var ctor = this.constructor;
        var options = ctor._classProperties.get(name) || defaultPropertyDeclaration$1;
        ...
        if (ctor._valueHasChanged(this[name], oldValue, options.hasChanged)) {
          ...
          if (options.reflect === true && !(this._updateState & STATE_IS_REFLECTING_TO_PROPERTY$1)) {
            if (this._reflectingProperties === undefined) {
              this._reflectingProperties = new Map();
            }
    ...
    key: "_valueHasChanged",
    value: function _valueHasChanged(value, old) {
      var hasChanged = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : notEqual$1;
      return hasChanged(value, old);
    }
    /**
     * Returns the property value for the given attribute value.
    ...
    value: function _propertyValueFromAttribute(value, options) {
      var type = options.type;
      var converter = options.converter || defaultConverter$1;
      var fromAttribute = typeof converter === 'function' ? converter : converter.fromAttribute;
      return fromAttribute ? fromAttribute(value, type) : value;
    }
    /**
    ...
    value: function _propertyValueToAttribute(value, options) {
      ...
      var type = options.type;
      var converter = options.converter;
      var toAttribute = converter && converter.toAttribute || defaultConverter$1.toAttribute;
      return toAttribute(value, type);
    }
  }, {
    key: "observedAttributes",
...
found at http://polymer.github.io/PATENTS.txt
*/
var supportsAdoptingStyleSheets$1 = 'adoptedStyleSheets' in Document.prototype && 'replace' in CSSStyleSheet.prototype;
...
var flattenStyles$1 = function flattenStyles(styles) {
  return styles.flat ? styles.flat(Infinity) : arrayFlat$1(styles);
};
...
function (_UpdatingElement) {
  ...
  _createClass(LitElement, [{
    ...
    value: function _getUniqueStyles() {
      ...
      if (Array.isArray(userStyles)) {
        var flatStyles = flattenStyles$1(userStyles); // As a performance optimization to avoid duplicated styling that can
        // occur especially when composing via subclassing, de-duplicate styles
        // preserving the last item in the list. The last item is kept to
        // try to preserve cascade order with the assumption that it's most
        // important that last added styles override previous styles.
...
function _templateObject2() {
  var data = _taggedTemplateLiteral(["\n              <span class=\"esds-rendered-example__label\">", "</span>\n            "]);
...
var EsdsRenderedExample =
/*#__PURE__*/
function (_Slotify) {
  _inherits(EsdsRenderedExample, _Slotify);

  function EsdsRenderedExample() {
    _classCallCheck(this, EsdsRenderedExample);

    return _possibleConstructorReturn(this, _getPrototypeOf(EsdsRenderedExample).apply(this, arguments));
  }
  ...
  return EsdsRenderedExample;
}(Slotify(LitElement$1));
...
var Directives$1 = directives$2.Directives;
...
var Options$2 = options$3.Options;
...
var Options$5 = options$4.Options;
var Output$2 = output.Output;
var InputScanner$3 = inputscanner.InputScanner;
var Directives$2 = directives$2.Directives;
...
var Directives$3 = directives$2.Directives;
...
var Options$8 = options$5.Options;
...
var html$2 = style_html;

var defaultOptions$2 = function defaultOptions() {
  return new Options$9();
};
html$2.defaultOptions = defaultOptions$2;

function style_html$1(html_source, options, js, css$1) {
  js = js || javascript;
  css$1 = css$1 || css;
  return html$2(html_source, options, js, css$1);
}

style_html$1.defaultOptions = html$2.defaultOptions;
var js = javascript;
var css$1 = css;
...
var isDirective$2 = function isDirective(o) {
 ...
 * True if the custom elements polyfill is in use.
 */
var isCEPolyfill$2 = window.customElements !== undefined && window.customElements.polyfillWrapFlushCallback !== undefined;
/**
 * Removes nodes, starting from `start` (inclusive) to `end` (exclusive), from
 * `container`.
 */

var removeNodes$2 = function removeNodes(container, start) {
  var end = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : null;
 ...
 * possible text in templates.
 */
var marker$2 = "{{lit-".concat(String(Math.random()).slice(2), "}}");
/**
 * An expression marker used text-positions, multi-binding attributes, and
 * attributes with markup-like text values.
 */

var nodeMarker$2 = "<!--".concat(marker$2, "-->");
var markerRegex$2 = new RegExp("".concat(marker$2, "|").concat(nodeMarker$2));
/**
 * Suffix appended to all bound attribute names.
 */
...
var Template$2 = function Template(result, element) {
  _classCallCheck(this, Template);
  ...
  while (partIndex < length) {
    ...
    /* Node.ELEMENT_NODE */
        ...
        if (node.hasAttributes()) {
          ...
          while (count-- > 0) {
            ...
            var name = lastAttributeNameRegex$2.exec(stringForPart)[2]; // Find the corresponding attribute
            // All bound attributes have had a suffix added in
            // TemplateResult#getHTML to opt out of special attribute
            // handling. To look up the attribute value we also need to add
            // the suffix.

            var attributeLookupName = name.toLowerCase() + boundAttributeSuffix$2;
            var attributeValue = node.getAttribute(attributeLookupName);
            node.removeAttribute(attributeLookupName);
            var statics = attributeValue.split(markerRegex$2);
            this.parts.push({
              type: 'attribute',
              index: index,
              name: name,
    ...
    /* Node.TEXT_NODE */
        ...
        if (data.indexOf(marker$2) >= 0) {
          ...
          var _strings = data.split(markerRegex$2);
          ...
          for (var _i = 0; _i < lastIndex; _i++) {
            ...
            if (s === '') {
              ...
              var match = lastAttributeNameRegex$2.exec(s);

              if (match !== null && endsWith$2(match[2], boundAttributeSuffix$2)) {
                s = s.slice(0, match.index) + match[1] + match[2].slice(0, -boundAttributeSuffix$2.length) + match[3];
              }
    ...
    /* Node.COMMENT_NODE */
        ...
        if (node.data === marker$2) {
          ...
          if (node.previousSibling === null || index === lastPartIndex) {
            ...
            _parent.insertBefore(createMarker$2(), node);
          }
          ...
          while ((_i2 = node.data.indexOf(marker$2, _i2 + 1)) !== -1) {
            // Comment node has a binding marker inside, make an inactive part
            // The binding won't work, but subsequent bindings will
            // TODO (justinfagnani): consider whether it's even worth it to
            // make bindings in comments work
...
var endsWith$2 = function endsWith(str, suffix) {
  var index = str.length - suffix.length;
  return index >= 0 && str.slice(index) === suffix;
};

var isTemplatePartActive$2 = function isTemplatePartActive(part) {
  return part.index !== -1;
}; // Allows `document.createComment('')` to be renamed for a
// small manual size-savings.

var createMarker$2 = function createMarker() {
  return document.createComment('');
};
/**
 * This regex extracts the attribute name preceding an attribute-position
...
var lastAttributeNameRegex$2 = /([ \x09\x0a\x0c\x0d])([^\0-\x1F\x7F-\x9F "'>=/]+)([ \x09\x0a\x0c\x0d]*=[ \x09\x0a\x0c\x0d]*(?:[^ \x09\x0a\x0c\x0d"'`<>=]*|"[^"]*|'[^']*))$/;
...
function () {
  ...
  _createClass(TemplateInstance, [{
    ...
    value: function _clone() {
      ...
      // polyfilled is the more ideal: Clone, Process, Adopt, Upgrade, Update,
      // Connect.
      var fragment = isCEPolyfill$2 ? this.template.element.content.cloneNode(true) : document.importNode(this.template.element.content, true);
      var stack = [];
      var parts = this.template.parts; // Edge needs all 4 parameters present; IE11 needs 3rd parameter to be null
...
var commentMarker$2 = " ".concat(marker$2, " ");
/**
 * The return type of `html`, which holds a Template and the values from
 * interpolated expressions.
 */

var TemplateResult$2 =
/*#__PURE__*/
function () {
  function TemplateResult(strings, values, type, processor) {
    _classCallCheck(this, TemplateResult);
  ...
  _createClass(TemplateResult, [{
    ...
    value: function getHTML() {
      ...
      for (var i = 0; i < l; i++) {
        ...
        var attributeMatch = lastAttributeNameRegex$2.exec(s);
        ...
        if (attributeMatch === null) {
          ...
          // $lit$ suffix to the name to opt-out of attribute-specific parsing
          // that IE and Edge do for style and certain SVG attributes.
          html += s.substr(0, attributeMatch.index) + attributeMatch[1] + attributeMatch[2] + boundAttributeSuffix$2 + attributeMatch[3] + marker$2;
        }
      }
...
var isIterable$2 = function isIterable(value) {
  return Array.isArray(value) || // tslint:disable-next-line:no-any
  !!(value && value[Symbol.iterator]);
};
/**
...
function () {
  ...
  _createClass(NodePart, [{
    ...
    value: function __commitTemplateResult(value) {
      ...
      if (this.value instanceof TemplateInstance$2 && this.value.template === template) {
        this.value.update(value.values);
      } else {
        // Make sure we propagate the template processor from the TemplateResult
        // so that we use its syntax extension, etc. The template factory comes
        // from the render function options so that it can control template
        // caching and preprocessing.
        var instance = new TemplateInstance$2(template, value.processor, this.options);
...
function (_AttributePart) {
  ...
  return PropertyPart;
}(AttributePart$2); // Detect event listener options support. If the `capture` property is read
// from the options object, then options are supported. If not, then the thrid
// argument to add/removeEventListener is interpreted as the boolean capture
// value so we should only pass the `capture` property.

var eventOptionsSupported$2 = false;
...
try {
  ...
  window.addEventListener('test', options$6, options$6); // tslint:disable-next-line:no-any

  window.removeEventListener('test', options$6, options$6);
} catch (_e) {}
...
var getOptions$2 = function getOptions(o) {
  return o && (eventOptionsSupported$2 ? {
    capture: o.capture,
    passive: o.passive,
    once: o.once
  } : o.capture);
...
var DefaultTemplateProcessor$2 =
/*#__PURE__*/
function () {
  function DefaultTemplateProcessor() {
    _classCallCheck(this, DefaultTemplateProcessor);
  ...
  _createClass(DefaultTemplateProcessor, [{
    ...
    value: function handleAttributeExpressions(element, name, strings, options) {
      ...
      if (prefix === '.') {
        var _committer = new PropertyCommitter$2(element, name.slice(1), strings);
      ...
      var committer = new AttributeCommitter$2(element, name, strings);
      return committer.parts;
    }
    /**
     * Create parts for a text-position binding.
...
var defaultTemplateProcessor$2 = new DefaultTemplateProcessor$2();
...
function templateFactory$2(result) {
  var templateCache = templateCaches$2.get(result.type);
  ...
  var key = result.strings.join(marker$2); // Check if we already have a Template for this key

  template = templateCache.keyString.get(key);

  if (template === undefined) {
    // If we have not seen this key before, create a new Template
    template = new Template$2(result, result.getTemplateElement()); // Cache the Template for this key
...
var render$4 = function render(result, container, options) {
  var part = parts$2.get(container);

  if (part === undefined) {
    removeNodes$2(container, container.firstChild);
    parts$2.set(container, part = new NodePart$2(Object.assign({
      templateFactory: templateFactory$2
    }, options)));
    part.appendInto(container);
  }
...
var html$4 = function html(strings) {
  for (var _len = arguments.length, values = new Array(_len > 1 ? _len - 1 : 0), _key = 1; _key < _len; _key++) {
    values[_key - 1] = arguments[_key];
  }

  return new TemplateResult$2(strings, values, 'html', defaultTemplateProcessor$2);
};
...
function removeNodesFromTemplate$2(template, nodesToRemove) {
  var content = template.element.content,
      parts = template.parts;
  var walker = document.createTreeWalker(content, walkerNodeFilter$2, null, false);
  var partIndex = nextActiveIndexInTemplateParts$2(parts);
  var part = parts[partIndex];
  var nodeIndex = -1;
  var removeCount = 0;
  var nodesToRemoveInTemplate = [];
...
var countNodes$2 = function countNodes(node) {
  var count = node.nodeType === 11
  /* Node.DOCUMENT_FRAGMENT_NODE */
  ? 0 : 1;
  var walker = document.createTreeWalker(node, walkerNodeFilter$2, null, false);
...
var nextActiveIndexInTemplateParts$2 = function nextActiveIndexInTemplateParts(parts) {
  var startIndex = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : -1;
...
function insertNodeIntoTemplate$2(template, node) {
  var refNode = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : null;
  var content = template.element.content,
      parts = template.parts; // If there's no refNode, then put node at end of template.
  // No part indices need to be shifted in this case.
...
var getTemplateCacheKey$2 = function getTemplateCacheKey(type, scopeName) {
  return "".concat(type, "--").concat(scopeName);
};

var compatibleShadyCSSVersion$2 = true;
...
var shadyTemplateFactory$2 = function shadyTemplateFactory(scopeName) {
  return function (result) {
    var cacheKey = getTemplateCacheKey$2(result.type, scopeName);
    var templateCache = templateCaches$2.get(cacheKey);
    ...
    var key = result.strings.join(marker$2);
    template = templateCache.keyString.get(key);
...
var TEMPLATE_TYPES$2 = ['html', 'svg'];
/**
 * Removes all style elements from Templates for the given scopeName.
 */

var removeStylesFromLitTemplates$2 = function removeStylesFromLitTemplates(scopeName) {
  TEMPLATE_TYPES$2.forEach(function (type) {
    var templates = templateCaches$2.get(getTemplateCacheKey$2(type, scopeName));
...
var prepareTemplateStyles$2 = function prepareTemplateStyles(scopeName, renderedDOM, template) {
  shadyRenderSet$2.add(scopeName); // If `renderedDOM` is stamped from a Template, then we need to edit that
  // Template's underlying template element. Otherwise, we create one here
  // to give to ShadyCSS, which still requires one while scoping.
  ...
  removeStylesFromLitTemplates$2(scopeName); // And then put the condensed style into the "root" template passed in as
  // `template`.
...
var render$5 = function render(result, container, options) {
  if (!options || _typeof(options) !== 'object' || !options.scopeName) {
    throw new Error('The `scopeName` option is required.');
  }

  var scopeName = options.scopeName;
  var hasRendered = parts$2.has(container);
  var needsScoping = compatibleShadyCSSVersion$2 && container.nodeType === 11
  /* Node.DOCUMENT_FRAGMENT_NODE */
  && !!container.host; // Handle first render to a scope specially...

  var firstScopeRender = needsScoping && !shadyRenderSet$2.has(scopeName); // On first scope render, render into a fragment; this cannot be a single
  // fragment that is reused since nested renders can occur synchronously.

  var renderContainer = firstScopeRender ? document.createDocumentFragment() : container;
  render$4(result, renderContainer, Object.assign({
    templateFactory: shadyTemplateFactory$2(scopeName)
  }, options)); // When performing first scope render,
  // (1) We've rendered into a fragment so that there's a chance to
  // `prepareTemplateStyles` before sub-elements hit the DOM
  // (which might cause them to render based on a common pattern of
  ...
  if (firstScopeRender) {
    var part = parts$2.get(renderContainer);
    parts$2["delete"](renderContainer); // ShadyCSS might have style sheets (e.g. from `prepareAdoptedCssText`)
    // that should apply to `renderContainer` even if the rendered value is
    // not a TemplateInstance. However, it will only insert scoped styles
    // into the document if `prepareTemplateStyles` has already been called
    // for the given scope name.

    var template = part.value instanceof TemplateInstance$2 ? part.value.template : undefined;
...
var notEqual$2 = function notEqual(value, old) {
  // This ensures (old==NaN, value==NaN) always returns false
  return old !== value && (old === old || value === value);
};
...
var microtaskPromise$2 = Promise.resolve(true);
var STATE_HAS_UPDATED$2 = 1;
var STATE_UPDATE_REQUESTED$2 = 1 << 2;
var STATE_IS_REFLECTING_TO_ATTRIBUTE$2 = 1 << 3;
var STATE_IS_REFLECTING_TO_PROPERTY$2 = 1 << 4;
var STATE_HAS_CONNECTED$2 = 1 << 5;
/**
 * The Closure JS Compiler doesn't currently have good support for static
 * property semantics where "this" is dynamic (e.g.
 * https://github.com/google/closure-compiler/issues/3177 and others) so we use
...
function (_HTMLElement) {
  ...
  function UpdatingElement() {
    ...
    _this._updateState = 0;
    _this._instanceProperties = undefined;
    _this._updatePromise = microtaskPromise$2;
    _this._hasConnectedResolver = undefined;
    /**
     * Map with keys for any properties that have changed since the last
     * update cycle with previous values.
  ...
  _createClass(UpdatingElement, [{
    ...
    key: "connectedCallback",
    value: function connectedCallback() {
      this._updateState = this._updateState | STATE_HAS_CONNECTED$2; // Ensure first connection completes an update. Updates cannot complete
      // before connection and if one is pending connection the
      // `_hasConnectionResolver` will exist. If so, resolve it to complete the
      // update, otherwise requestUpdate.
    ...
    value: function _attributeToProperty(name, value) {
      ...
      if (propName !== undefined) {
        var options = ctor._classProperties.get(propName) || defaultPropertyDeclaration$2; // mark state reflecting
    ...
    value: function _requestUpdate(name, oldValue) {
      ...
      if (name !== undefined) {
        var ctor = this.constructor;
        var options = ctor._classProperties.get(name) || defaultPropertyDeclaration$2;
        ...
        if (ctor._valueHasChanged(this[name], oldValue, options.hasChanged)) {
          ...
          if (options.reflect === true && !(this._updateState & STATE_IS_REFLECTING_TO_PROPERTY$2)) {
            if (this._reflectingProperties === undefined) {
              this._reflectingProperties = new Map();
            }
    ...
    key: "_valueHasChanged",
    value: function _valueHasChanged(value, old) {
      var hasChanged = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : notEqual$2;
      return hasChanged(value, old);
    }
    /**
     * Returns the property value for the given attribute value.
    ...
    value: function _propertyValueFromAttribute(value, options) {
      var type = options.type;
      var converter = options.converter || defaultConverter$2;
      var fromAttribute = typeof converter === 'function' ? converter : converter.fromAttribute;
      return fromAttribute ? fromAttribute(value, type) : value;
    }
    /**
    ...
    value: function _propertyValueToAttribute(value, options) {
      ...
      var type = options.type;
      var converter = options.converter;
      var toAttribute = converter && converter.toAttribute || defaultConverter$2.toAttribute;
      return toAttribute(value, type);
    }
  }, {
    key: "observedAttributes",
...
found at http://polymer.github.io/PATENTS.txt
*/
var supportsAdoptingStyleSheets$2 = 'adoptedStyleSheets' in Document.prototype && 'replace' in CSSStyleSheet.prototype;
...
var flattenStyles$2 = function flattenStyles(styles) {
  return styles.flat ? styles.flat(Infinity) : arrayFlat$2(styles);
};
...
function (_UpdatingElement) {
  ...
  _createClass(LitElement, [{
    ...
    value: function _getUniqueStyles() {
      ...
      if (Array.isArray(userStyles)) {
        var flatStyles = flattenStyles$2(userStyles); // As a performance optimization to avoid duplicated styling that can
        // occur especially when composing via subclassing, de-duplicate styles
        // preserving the last item in the list. The last item is kept to
        // try to preserve cascade order with the assumption that it's most
        // important that last added styles override previous styles.
...
function (_LitElement) {
  ...
  _createClass(EsdsCodeSnippet, [{
    ...
    value: function renderCopyButton() {
      ...
      var copyButton = html$4(_templateObject$1(), this.copyButtonText);

      if (this.copyable === 'true') {
        return html$4(_templateObject2$1(), this.codeCopiedText, this.copyCodeToClipboard, copyButton);
      } else {
        return '';
      }
    }
    ...
    value: function renderToolbar() {
      ...
      if (toolbarActions.length > 0) {
        output = html$4(_templateObject3(), toolbarActions);
      }
...
function (_LitElement) {
  ...
  function EsdsExampleCodePair() {
    ...
    _classCallCheck(this, EsdsExampleCodePair);

    _this = _possibleConstructorReturn(this, _getPrototypeOf(EsdsExampleCodePair).call(this)); // Alias EsdsRenderedExample for usage within this component

    if (!window.customElements.get('esds-ecpair-rendered-example')) {
      window.customElements.define('esds-ecpair-rendered-example', EsdsRenderedExample);
